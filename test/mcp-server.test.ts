...
describe("quant-mcp server layer", () => {
  ...
  it("protocol round-trip: client calls validate_strategy through MCP", async () => {
    ...
    expect(tools.tools.map((t) => t.name).sort()).toEqual(
      [
        // v1 분석 8툴 + scan_universe + allocate_portfolio + list_events
        ...
        // v2.5 라이브 9툴(+주문 역쿼리 2종 — audit P1-16)
        "live_status", "get_positions", "get_balance", "place_order", "place_protective", "get_protective", "cancel_protective", "get_open_orders", "get_order_status",
      ].sort()
    );
