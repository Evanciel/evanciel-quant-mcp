...
describe("quant-mcp server layer", () => {
  ...
  it("protocol round-trip: client calls validate_strategy through MCP", async () => {
    ...
    expect(tools.tools.map((t) => t.name).sort()).toEqual(
      [
        // v1 분석 8툴 + scan_universe(스크리닝)
        "backtest", "backtest_short", "derivatives_signal", "detect_regime", "portfolio_risk", "strategy_factory", "suggest_position_size", "validate_strategy", "scan_universe",
        // v2 봇/전략/대시보드 7툴
