...
세 단계 모두 **같은 엔진**이 돌립니다 — 신호 판단과 포지션 사이징은 백테스트와 라이브가 말 그대로 같은 코드입니다. 단, **체결은 다릅니다**: 라이브는 닫힌 봉마다 실제 거래소에 시장가 주문을 내므로 슬리피지·부분체결·수수료는 시뮬레이션과 정확히 일치하지 않습니다.
...
- 🔬 **거짓 발견 필터** — Deflated / Probabilistic Sharpe(DSR/PSR), 70/30 홀드아웃 OOS 게이팅. *팩토리 툴이 대부분의 후보를 기각하는 건 의도된 동작이지 버그가 아닙니다.*
- 🧩 **표현력** — 조합형 전략 트리(지표 × 레짐 × 세션 × 페어 × 멀티타임프레임 × 캘린더 이벤트 × 스크리너)를 **하나의 검증 스키마**로, 그리고 **backtest ≡ live 신호 동등성**(신호·사이징 판단은 같은 순수 함수 — 주문 *체결*은 위에 적었듯 다름).
...
quant-mcp는 stdio MCP 서버입니다 — **MCP를 지원하는 어떤 에이전트**(Claude Desktop, Claude Code, Cursor, Continue 등)든 27개 툴을 쓸 수 있습니다. API 키 불필요(데이터는 바이낸스 공개 REST).
...
서버는 stderr에 `quant-mcp server ready (stdio) — 27 tools`를 출력합니다.
...
| `backtest` | 백테스트 + 70/30 홀드아웃 OOS + PSR(과적합 탐지). |
...
| `create_bot` | 저장된 전략으로 봇 생성(기본 paper; `mode:live`는 키 + 마스터스위치 게이트 통과 후에만 실거래, 미통과 시 페이퍼 폴백 — 생성 시 사전승인, 주문별 토큰 없음). |
...
| `get_open_orders` | 심볼의 미체결(상주) 주문 목록 — 지정가 잔존·고아 주문 점검(읽기전용, 현재 Binance만). |
| `get_order_status` | `orderId`/`clientOrderId`로 주문 상태 역쿼리 — 주문 후 체결 검증(읽기전용). |
...
`save_strategy` → `create_bot` → `start_bot` 으로 봇을 돌리면, 매 닫힌 봉마다 **같은 백테스트 엔진**으로 재평가합니다 — **신호·사이징 결정은 백테스트와 동일**합니다. 단 *체결*은 다릅니다: 라이브는 종가 기준 시장가 주문이라 슬리피지·부분체결·레이턴시는 모델링하지 않습니다(결정은 일치, 체결은 다름). 상태는 로컬 `node:sqlite` 스토어에 저장 — 계정도, 클라우드도 없습니다.

`open_dashboard`는 `127.0.0.1`에 실시간 HTML 대시보드를 띄웁니다(1회용 부트스트랩 토큰을 **HttpOnly 세션쿠키로 교환** — 접속 후 주소창·페이지에 토큰 미노출, 차트 라이브러리는 `/vendor` 셀프호스팅=서드파티 스크립트 0, 바이낸스 공개 WS로 실시간 미실현손익). **일반인용**으로 만들었습니다: 쉬운 말 전략 요약("상승 추세일 때만 과매도에서 매수"), 🟢 수익 / 🔴 손실 / ⚪ 대기 배지, 실현·미실현 손익, 멀티심볼 스캐너 포지션 — 전문 표기는 "자세히" 토글로.
...
**수동 매매 & 보호주문(BYOK, testnet 게이트):** 봇 카드에서 바로 시장가/지정가 **매수/매도**, 그리고 **차트에서 익절/손절선을 끌어** 바이낸스 현물 **OCO**(한쪽 체결 시 다른쪽 자동취소: 익절 체결되면 손절 자동취소, 반대도) 주문을 겁니다. 이 수동 주문들은 봇과 *동일한* 머니패스를 거칩니다 — `liveGate`(마스터 스위치 OFF면 testnet/mock만) → 서버에서 실보유 수량·방향 재검증(클라 값 불신) → 노셔널 캡 → 감사로그 — 여기에 더해 수동 주문에는 **2단계 확인토큰**(미리보기 → 확정, 해시 바인딩, 단일사용, 5분 TTL)이 추가됩니다(자율 봇은 주문별 토큰 없이 게이트·하드리밋·멱등으로만 통제). 이 수동 기능들은 대시보드에서만 동작하며 메인넷은 **기본 OFF**.

**거래소 실계정 동기화(읽기 전용):** 브로커별로 **실계정 패널** — 거래소 실잔고·실보유·상주 **OCO**(같은 안전경로로 원클릭 취소) — 과 **페이퍼 vs 거래소 드리프트 배지**(페이퍼 봇 장부가 실보유와 얼마나 벌어졌는지 정량화)를 함께 보여줍니다. 키는 브라우저로 절대 반환되지 않고, 패널은 `getAccount`를 60초마다 폴링하며 주문은 절대 내지 않습니다.

**이벤트 알림(Slack / Discord 웹훅):** 대시보드 알림 피드가 봇 이벤트(진입·청산+실현손익·**오류/중지** 전이)를 실시간으로 보여주고, **Slack/Discord 웹훅**으로 전송할 수 있습니다. 웹훅 URL은 시크릿으로 취급(마스킹 저장, 에코 0)하며 엄격한 **SSRF 게이트**를 통과합니다 — HTTPS 전용, Slack/Discord 호스트 화이트리스트, IP 리터럴/userinfo/비443 포트 거부, 웹훅 경로 형태 검증, `redirect: 'error'` — 내부 주소로 유도 불가. 전송은 봇+이벤트 단위로 디바운스되어 폭주를 막습니다.
...
- **거짓 발견 게이트:** 70/30 홀드아웃 OOS, PSR/DSR(deflated Sharpe), `strategy_factory`.
...
src/mcp-server/   stdio MCP 서버 + 27개 툴
```
...
- ✅ MCP 서버 + 27개 툴(분석, 스크리닝, 포트폴리오, 이벤트, 봇, 라이브 BYOK)
...
- ✅ 거래소 실계정 동기화(읽기 전용): 실잔고/실보유/상주 OCO + 페이퍼 vs 거래소 드리프트 배지 — 키는 호스트 밖으로 안 나감
- ✅ 이벤트 알림: 대시보드 피드 + Slack/Discord 웹훅(엄격한 **SSRF 게이트**: HTTPS 전용·호스트 화이트리스트·리다이렉트 차단) + 이벤트별 디바운스
...
- **2단계 주문 확인(수동 주문)** — `place_order` / `place_protective`는 fail-closed: 프리뷰가 토큰 반환, 동일 인자 + 토큰이어야 실행.
- **자율 봇 vs 수동 주문** — `mode:live` 봇은 **주문별 토큰이 없습니다**: `create_bot` 시 한 번 사전승인되고 마스터 스위치 + 하드리밋 + 멱등으로 통제됩니다(동일한 단일 머니패스). 주문별 2단계 토큰은 수동 대시보드/MCP 주문 툴에만 적용됩니다.
- **대시보드** — `127.0.0.1` 전용 바인딩, 1회용 부트스트랩 토큰 → HttpOnly 세션쿠키(이후 주소창·페이지에 토큰 미노출), Host+Origin 검사, 차트 라이브러리 셀프호스팅(서드파티 스크립트 0), **실거래 켜기=미리보기→확정 2단계**(끄기는 원클릭 유지).
...
| **backtest ≡ live(동등성)** | *판단*(신호·사이징)은 백테스트와 라이브가 같은 코드. 체결까지 같진 않음 — 라이브는 실제 시장가 주문이라 슬리피지·수수료가 시뮬레이션과 다름. |
