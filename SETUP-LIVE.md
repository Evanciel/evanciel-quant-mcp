...
- **보호주문은 현물 Binance OCO만.** *수동* `place_protective`만 현물 롱에 실제 OCO(한쪽 체결 시 다른쪽 자동취소)를 겁니다. **자율 봇은 OCO가 아니라 독립 상주주문 2개(STOP + 익절)** 를 `syncProtective`/`planProtectiveOrders`로 배치합니다(둘은 다른 메커니즘). 선물·타브로커는 아직 상주 보호주문이 없습니다. (OCO 응답 파서는 `orderListId` 부재/`orderReports` 빈 배열이 성공으로 둔갑하지 않도록 별도 강화 진행 중.)
