...
npx quant-mcp setup
```
브로커를 고르고 키를 입력하면 끝. 입력 중 시크릿은 화면에 `*`로 가려지고, 저장 후엔 마스킹으로만 표시됩니다. MCP 서버를 재시작하면 자동 로드됩니다.

**방법 B. 대시보드 폼 (브라우저에서)**
`open_dashboard`로 대시보드를 연 뒤 우측 상단 **⚙️ API 키 설정** → 브로커별 칸에 입력 → 저장. `type=password`로 가려지고, 저장 즉시 적용(같은 프로세스라 재시작 불필요). 127.0.0.1 전용 + 런치별 토큰 + Host 검증으로 보호되며, 저장한 키는 **다시 읽어올 수 없습니다**(마스킹만).
